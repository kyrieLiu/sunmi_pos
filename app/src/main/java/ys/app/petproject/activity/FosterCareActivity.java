...
import ys.app.petproject.adapter.FosterCareAdapter;
...
public class FosterCareActivity extends AppCompatActivity implements View.OnClickListener {
    private EditText et_close, owner_name, owner_phone, partyB_name, partyB_phone, pet_name, pet_kind,pet_varieties, pet_age, pet_weight,
            pet_coatColor, pet_remark, startTime, endTime, allTime,date_tv, owner_year,
            owner_month, owner_day, partyB_year, partyB_month, partyB_day;
    ...
    private String showAge, showWeight, showColor,showStartTime,showEndTime,showAllTime;//
...
//        owner_year= (EditText) findViewById(R.id.et_foster_care_owner_year);
//        owner_month= (EditText) findViewById(R.id.et_foster_care_owner_month);
//        owner_day= (EditText) findViewById(R.id.et_foster_care_owner_day);
//        partyB_year = (EditText) findViewById(R.id.et_foster_care_partyB_year);
//        partyB_month = (EditText) findViewById(R.id.et_foster_care_partyB_month);
//        partyB_day = (EditText) findViewById(R.id.et_foster_care_partyB_day);
    ...
    private void initData() {
        SummitOrderInfo orderInfo = (SummitOrderInfo) getIntent().getExtras().getSerializable(Constants.intent_info);
        ...
        switch (v.getId()) {
            ...
            case R.id.iv_hairdressing_lifes_arrow:
                if (rv_lifes_clip.getVisibility() == View.VISIBLE) {
                    ...
                    iv_lifes_visible.setImageResource(R.mipmap.mei_rong_down);
                }
                break;
    ...
    public void clickConfirm() {
        ...
        showStartTime = TextUtils.isEmpty(getText(startTime))?"______":getText(startTime);
        showEndTime = TextUtils.isEmpty(getText(endTime))?"______":getText(endTime);
        showAllTime = TextUtils.isEmpty(getText(allTime))?"____":getText(allTime);
        ...
        builder1.append("超过约定的时间甲方按天数向乙方补交费用，超过约定时间，如果甲方没有电话或其他联系方式告知乙方要求继续寄养，乙方有权将寄养宠物按无主宠物处理" +
        ...
        printContent("(客户存根)",builder, builder1, builder2, builder3, builderTime, builderClothes, builderLifes);
        printContent("(商户存根)",builder, builder1, builder2, builder3, builderTime, builderClothes, builderLifes);
    ...
    private void printContent(String diff,StringBuilder builder, StringBuilder builder1, StringBuilder builder2, StringBuilder builder3, StringBuilder builderTime, StringBuilder builderClothes, StringBuilder builderLifes) {
        if ("M1".equals(Build.MODEL)) {
            BlueToothPrintUtil mAidlUtil = BlueToothPrintUtil.getInstance();
            mAidlUtil.printTitle("寄养协议"+diff);
            ...
            mAidlUtil.printAgreement("\n临时寄养协议: ", 28, false, true);
            mAidlUtil.printAgreement("\n一、乙方为甲方提供有偿的临时宠物寄养服务，甲方将委托寄养宠物交由乙方临时寄养，寄养时间为", 24, false, false);
            mAidlUtil.printAgreement(showStartTime, 24, true, false);
            mAidlUtil.printAgreement("至", 24, false, false);
            mAidlUtil.printAgreement(showEndTime, 24, true, false);
            mAidlUtil.printAgreement("，预计", 24, false, false);
            mAidlUtil.printAgreement(showAllTime, 24, true, false);
            mAidlUtil.printAgreement("天。", 24, false, false);
            mAidlUtil.printAgreement(builder1.toString(), 24, false, false);
            mAidlUtil.printAgreement(builder2.toString(), 20, false, false);
            mAidlUtil.printAgreement(builder3.toString(), 24, false, false);
            mAidlUtil.printAgreement("\n备注: " + getText(pet_remark), 24, false, false);
