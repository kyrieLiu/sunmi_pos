...
//    }
     ...
     *负载均衡
     */
    public ApiClient<T> reqOtherURL(@NonNull String apiMethodName, Map<String, String> params, @NonNull ApiRequest.RespDataType dataType) {
        ApiRequest<ApiService> apiRequest;
        if (dataType == ApiRequest.RespDataType.RESPONSE_XML) {
            apiRequest = ApiRequest.getDefaultInstance();
        } else if (dataType == ApiRequest.RespDataType.RESPONSE_JSON) {
            apiRequest = new ApiRequest.Builder(AppApplication.getAppContext(), Constants.base_other_url).respDataType(ApiRequest.RespDataType.RESPONSE_JSON).build();
        } else {
            Logger.w("Failure to create the 'ApiRequest'.");
            return this;
        }
