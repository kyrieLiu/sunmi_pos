...
public class AddVipCardViewModel extends BaseActivityViewModel {
    ...
    public void init() {
        ...
        msp.setSpan(new ForegroundColorSpan(mActivity.getResources().getColor(R.color.text_money)), 2, 4, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); //设置前景色为洋红色
        msp.setSpan(new UnderlineSpan(), 2, 4, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        msp.setSpan(new ForegroundColorSpan(mActivity.getResources().getColor(R.color.text_money)), 14, 17, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); //设置前景色为洋红色
        msp.setSpan(new UnderlineSpan(), 14, 17, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        msp.setSpan(new ForegroundColorSpan(mActivity.getResources().getColor(R.color.text_money)), 28, 31, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); //设置前景色为洋红色
    ...
    public void clickButton(View v) {
        ...
        if (mInfo == null) {//新增
            ...
            if (goodTypeInfos != null && !goodTypeInfos.isEmpty()) {
                Logger.e("new Gson().toJson(goodTypeInfos) = " + new Gson().toJson(goodTypeInfos));
                params.put("commodityDiscount", new Gson().toJson(goodTypeInfos));
            }

            if (serviceTypeInfos != null && !serviceTypeInfos.isEmpty()) {
                Logger.e("new Gson().toJson(serviceTypeInfos) = " + new Gson().toJson(serviceTypeInfos));
                params.put("productDiscount", new Gson().toJson(serviceTypeInfos));
            }
