...
import ys.app.petproject.shangmi.printer.gputil.DeviceConnFactoryManager;
import ys.app.petproject.shangmi.printer.gputil.PrinterCommand;
...
import static ys.app.petproject.shangmi.printer.gputil.Constant.ACTION_USB_PERMISSION;
import static ys.app.petproject.shangmi.printer.gputil.DeviceConnFactoryManager.ACTION_QUERY_PRINTER_STATE;
import static ys.app.petproject.shangmi.printer.gputil.DeviceConnFactoryManager.CONN_STATE_FAILED;
...
//                    }
    ...
    public void printOrderPayInformation(OrderInfo mOrderInfo) {
        ...
        if (orderDetailedList != null && !orderDetailedList.isEmpty()) {
            ...
            String str2 = headerRow2 + getBlankBySize(four_row2 + 3 - length(headerRow2));
            String str3 = headerRow3 + getBlankBySize(four_row2 + 5 - length(headerRow3));
            ...
            for (OrderInfo.OrderDetailedListBean bean : orderDetailedList) {
                ...
                if (nameSize > tempRow) {
                    ...
                    String newLineSecond = count + getBlankBySize(four_row2 +2 - length(count));
                    String newLineThird = vipPrice + getBlankBySize(four_row2 + 5 - length(vipPrice));
                    ...
                    String rowSecond = count + getBlankBySize((int) (four_row2 + 2 - length(count)));
                    String rowThird = vipPrice + getBlankBySize((int) (four_row2 + 5 - length(vipPrice)));
                    // 最后直接换行就可以了
                    String rowEnd = price + "\n";
                    //设置小票是否展示会员价
                    String rowAll = "";
